...
#[repr(transparent)]
#[derive(Debug)]
pub(crate) struct Public<'m>(Alias<'m>);
...
impl<'m> quote::ToTokens for Public<'m> {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        let alias_tokens = if let alias::Index::Primary = index {
            let mut aliased_type_no_generics = target.aliased_type.clone();
            ...
            quote!(#super_visibility use #aliased_type_no_generics as #ident;)
        } else {
            ...
            let simple_path = if !self.0.is_generic_parameter_type() {
                if let Some(_qpath) = &aliased_type.qself {
                    None
                } else {
                    Some(&aliased_type.path)
                }
            } else {
                None
            };
            ...
            if let Some(simple_path) = simple_path {
                ...
                quote_spanned! { span =>
                    #super_visibility use self::exact::#ident_internal as #ident;
                }
            }
        };
