...
impl Command {
    ...
    // /// Similar to [Command::apply], except if `macro_path` does not include a macro, `fallback` will be expanded instead.
    // /// (Standard macro limitations apply, such as not using `$` directly.)
    ...
    // /// otherwise, `fallback` is expanded instead.
